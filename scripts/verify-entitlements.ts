// Verifies lib/entitlements.ts's tier resolution — three layered rules
// (real tier + lapse policy, beta-tester grant, dev override) that are
// easy to get subtly wrong. Run with `npx tsx scripts/verify-entitlements.ts`.

import { effectiveTier, isBetaTester, isDevAccount } from "../lib/entitlements";
...
assertEqual(
  "free stays free",
  effectiveTier({ tier: "free", proCurrentPeriodEnd: null, email: "x@example.com", devTierOverride: null }),
  "free"
);
assertEqual(
  "plus stays plus",
  effectiveTier({ tier: "plus", proCurrentPeriodEnd: null, email: "x@example.com", devTierOverride: null }),
  "plus"
);
assertEqual(
...
assertEqual(
  ...
  effectiveTier({ tier: "free", proCurrentPeriodEnd: null, email: "someone-else@gmail.com", devTierOverride: "pro" }),
  "free"
);
assertEqual(
  "null devTierOverride falls through to real tier even for the dev account",
  effectiveTier({ tier: "plus", proCurrentPeriodEnd: null, email: "watkins.jonathan@gmail.com", devTierOverride: null }),
  "plus"
);
